...
import com.han.guava.newcache.other.CacheLoaderTest;
...
/**
 * @Author: hanyf
 * @Description:，我利用了它的 N 个时间内不写入数据时缓存就清空的特点，在每次读取数据时判断异常信息是否大于 X 即可
 * @Date: 2018/11/9 19:12
 */
public class EmptiedCache {

    private final static Logger LOGGER = LoggerFactory.getLogger(EmptiedCache.class);
    ...
    /**   
     *
     *功能描述 首先是构建了 LoadingCache 对象，在 N 分钟内不写入数据时就回收缓存（当通过 Key 获取不到缓存时，默认返回 0）。然后在每次消费时候调用 checkAlert() 方法进行校验，这样就可以达到上文的需求。
     ...
     * @date 2018/11/9
     * @param [] 
     * @return com.google.common.cache.LoadingCache
     */
    ...
    public static void main(String[] args) throws InterruptedException {
        EmptiedCache cacheLoaderTest = new EmptiedCache() ;
